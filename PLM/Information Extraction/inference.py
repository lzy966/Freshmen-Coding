...
if __name__ == "__main__":
    ...
    device = 'cpu'                                       # 指定GPU设备
    saved_model_path = 'D:/Jupyter notebook file/硕士/PLM/Information Extraction/model_640/'     # 训练模型存放地址
    tokenizer = AutoTokenizer.from_pretrained(saved_model_path) 
    # model = torch.load(os.path.join(saved_model_path, 'uie_model.pt'))
    # 先加载模型结构
    model = AutoModel.from_pretrained("D:/Jupyter notebook file/硕士/PLM/Information Extraction/model_640/")  
    # 加载保存的参数
    state_dict = torch.load(os.path.join(saved_model_path, 'uie_model.pt'), map_location=device)
