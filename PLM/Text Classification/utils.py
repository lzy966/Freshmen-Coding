...
def convert_example(examples: dict, tokenizer, max_seq_len: int):
    ...
    for example in examples['text']:
        try:
            ...
            label = int(label)
            # label = label.strip()  # 清理多余的空格
            # # 使用 label2id 映射字典来将标签从字符串转为整数
            # if label in label2id:
            #     label = label2id[label]
            # else:
            #     print(f"[WARNING] Unknown label: {label}, skipping this example.")
            #     continue
            ...
            print(f"[ERROR] Skipping malformed line: {example} -> {traceback.format_exc()}")
            continue
