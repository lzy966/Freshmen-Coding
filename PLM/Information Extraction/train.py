...
sys.argv = [
    ...
    "--train_path", "D:/Data/master/Freshmen Coding Assignment/NLP/DuIE/train.txt",
    "--dev_path", "D:/Data/master/Freshmen Coding Assignment/NLP/DuIE/dev.txt",
    "--save_dir", "D:/Jupyter notebook file/硕士/PLM/Information Extraction",
    "--img_log_dir", "D:/Jupyter notebook file/硕士/PLM/Information Extraction",
...
#     print(os.path.join(args.pretrained_model, 'pytorch_model.bin'))
#     model_name = "nghuyong/ernie-3.0-base-zh"
#     save_path = "uie-base-zh"
#     # 加载 tokenizer 和 encoder
...
#     # model = torch.load(os.path.join(args.pretrained_model, 'pytorch_model.bin'),weights_only=False)         # 加载预训练好的UIE模型，模型结构见：model.UIE()
...
#             print("start_prob dtype:", start_prob.dtype)
#             print("start_prob device:", start_prob.device)
...
#                 convert_func
#             )
#             model = torch.load(os.path.join(args.pretrained_model, 'pytorch_model.bin'),weights_only=False)        # 重新加载预训练模型
#             model.to(args.device)
#             optimizer, lr_scheduler = get_optimizer_and_scheler(model, train_dataloader)
...
def train():
    ...
    # 保存 tokenizer 和模型权重
    tokenizer.save_pretrained(save_path)
    torch.save(model.state_dict(), os.path.join(save_path, "uie_model.pt"))
    ...
    for epoch in range(1, args.num_train_epochs + 1):
        for batch in train_dataloader:
            ...
            # 确保 target 是 float tensor
            start_ids = batch['start_ids'].float().to(args.device)
            end_ids = batch['end_ids'].float().to(args.device)
            ...
            if global_step % args.logging_steps == 0:
                ...
                loss_avg = sum(loss_list) / len(loss_list)
                writer.add_scalar('train_loss', loss_avg, global_step)
                print(f"global step {global_step}, epoch {epoch}, loss: {loss_avg:.5f}, speed: {args.logging_steps / time_diff:.2f} step/s")
                tic_train = time.time()

            # 验证与保存
            if global_step % args.valid_steps == 0:
                cur_save_dir = os.path.join(args.save_dir, f"model_{global_step}")
                os.makedirs(cur_save_dir, exist_ok=True)

                # 保存 UIE 模型权重
                torch.save(model.state_dict(), os.path.join(cur_save_dir, "uie_model.pt"))
                tokenizer.save_pretrained(cur_save_dir)
                ...
                # 更新最优模型
                if f1 > best_f1:
                    print(f"Best F1 updated: {best_f1:.5f} --> {f1:.5f}")
                    best_f1 = f1
                    best_save_dir = os.path.join(args.save_dir, "model_best")
                    os.makedirs(best_save_dir, exist_ok=True)
                    torch.save(model.state_dict(), os.path.join(best_save_dir, "uie_model.pt"))
